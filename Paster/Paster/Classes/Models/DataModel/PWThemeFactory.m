...
{
    NSMutableArray * themeNames = [Configurer themeNames];
...
{
    NSDictionary *pastersDict = [themeDict objectForKey:@"Pasters"];
    
    //读取主题的贴纸图片，进行分割
    NSInteger column = [[pastersDict objectForKey:@"Column"] intValue];
    NSInteger row = [[pastersDict objectForKey:@"Row"] intValue];
    
    NSString *imagePath = [Configurer imageFolderPath:theme.themeName];
    UIImage *pasterImage = [UIImage imageWithContentsOfFile:[imagePath stringByAppendingPathComponent:@"PasterImage.png"]];
    NSMutableArray *pasterImageArray = [ImageSpliter separateImage:pasterImage ByX:column andY:row];
    
    
    //初始化贴纸的基本信息
    NSArray *pasterArray = [pastersDict objectForKey:@"PasterArray"];
    
    for (NSDictionary *pasterDict in pasterArray) {
        NSInteger index = [pasterArray indexOfObject:pasterDict];
...
//        NSString *fileName = [pasterDict objectForKey:@"FileName"];
        UIImage *image = [pasterImageArray objectAtIndex:index];

...
- (id)init
{
    ...
    if (self)
    {
        ...
        pointUnit = [[GCPointUnit alloc]init];
...
-(void)draw_extension_line:(CGContextRef)context
{
    for(int i=0; i<constraintList.count; i++)
    {
        ...
        {
            //if(!(self.pointUnit.start == (GCLineGraph*)constraintTemp.relatedGraph.line.start))
        }
    }
}
